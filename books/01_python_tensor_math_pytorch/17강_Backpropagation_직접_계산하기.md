...
제14강에서 Chain Rule을, 제16강에서 Forward와 cache를 배웠다.  
이번 강의는 그 둘을 합쳐, **작은 네트워크의 모든 편미분을 손으로** 구한다.
...
이 강의는 1권의 심장부에 가깝다. 숫자가 많아도 건너뛰지 말 것.
...
#### 4.2 전체 파이프라인 (Loss → … → Parameter.grad)
...
### 7. 작은 숫자로 직접 계산하기 — 모든 편미분

목표: 숫자로 \(\nabla L\)을 전부 구한다.

#### 7.1 Step A — \(\partial L/\partial\hat{y}\)
...
#### 7.4 Step D — 은닉 Activation으로 흘리기
...
#### 7.5 Step E — ReLU를 지나 Pre-activation으로
...
### 8. 경로별로 Chain Rule 펼치기 (한 Weight만 확대)
...
### 10. Computational Graph에서의 Backward 스케치
