...
$$
\bigl(\mathrm{sum}(A,\ \mathrm{axis}=1)\bigr)_i = \sum_{j=1}^{n} A_{ij}
\ \in\ \mathbb{R}^{m}
$$
...
logits $\mathbf{z}\in\mathbb{R}^{V}$에 대해
...
를 **각 토큰 위치마다** 독립적으로 계산합니다. 배치·시퀀스가 있으면 Shape는 `(B, T, V) → (B, T, V)`이고, 합은 $V$축에서만 1이 됩니다.

$$
\sum_{k=1}^{V} \mathrm{softmax}(Z)_{b,t,k} = 1
\quad\forall\, b,t
$$
...
$$
\hat{a}_i = \frac{a_i}{\|a\|_2},\qquad
\|a\|_2=\sqrt{\sum_j a_j^2}
$$

임베딩 유사도 비교 전에 자주 씁니다.
...
print(a_hat, np.sum(a_hat**2))  # 단위 벡터, 제곱합 ≈ 1
```

### Shape 추적 연습

임베딩 조회 후 Linear를 거친다고 가정합니다.
...
\;\rightarrow\;
Z = E W \in\mathbb{R}^{B\times T\times V}
$$

NumPy로도 `(B*T, d) @ (d, V)` 후 reshape로 같은 결과를 낼 수 있습니다.
