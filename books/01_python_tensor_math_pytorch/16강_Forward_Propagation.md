...
$$
L = \frac{1}{2B}\sum_{n=1}^{B}(\hat{y}_n-y_n)^2 = \frac{1}{4}\Big[(0.21-1)^2+(0.20-1)^2\Big] = \frac{1}{4}(0.6241+0.64) = 0.316025
$$
...
$W2_{11}$만 $-1.0$부터 $1.0$까지 0.1 간격으로 바꾸며 Loss를 기록해 꺾은선으로 그려 봅니다.  
Forward만으로도 “파라미터가 Loss를 어떻게 바꾸는지” 지형을 엿볼 수 있습니다. 최솟값으로 가는 방법은 12강·17강의 Gradient입니다.

### 실습 6 — Sigmoid 출력

예제 A에서 `forward_binary`의 $p=\sigma(0.21)$를 계산하고, $y=1$일 때 BCE

$$
L_{\mathrm{BCE}} = -\big(y\log p + (1-y)\log(1-p)\big)
$$
