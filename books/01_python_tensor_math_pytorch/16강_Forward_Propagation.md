...
<!-- visual-example-16 -->
## 숫자로 따라가기 — Forward 한 바퀴

![그림 16-1](images/fig16-01.png)

15강과 같은 2-2-1에서 Forward만 따라갑니다.

| 기호 | 값 | 단계 |
|---|---|---|
| $\mathbf{x}$ | $[1.0,\ 0.5]$ | 입력 |
| $\mathbf{z}^{(1)}$ | $[0.30,\ 0.35]$ | $W^{(1)}\mathbf{x}+\mathbf{b}^{(1)}$ |
| $\mathbf{a}^{(1)}$ | $[0.30,\ 0.35]$ | ReLU |
| $\hat{y}$ | $0.21$ | $W^{(2)}\mathbf{a}^{(1)}+b^{(2)}$ |
| $y$ | $1.0$ | 타깃 |
| $L$ | $0.31205$ | $\frac12(\hat{y}-y)^2$ |

핵심 한 줄:

$$
\hat{y}=0.5\cdot 0.30+(-0.4)\cdot 0.35+0.2=0.21
$$

Backward(17강)를 위해 $\mathbf{z}^{(1)},\mathbf{a}^{(1)}$를 **캐시**에 남깁니다. LLM 추론도 결국 이 Forward의 반복입니다.
