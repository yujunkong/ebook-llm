...
제17강에서 Backprop을 손으로, 제18강에서 NumPy로 구현했다.  
제19강에서 PyTorch Tensor를 익혔다. 이번 강의는 그 위에 **Autograd**를 올려, 프레임워크가 Gradient를 자동으로 채우는 원리를 연결한다.
...
#### 4.7 torch.no_grad / inference_mode
...
#### 4.9 Loss → Graph → Chain Rule → Gradient → Parameter.grad
