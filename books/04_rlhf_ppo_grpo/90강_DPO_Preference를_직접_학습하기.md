...
## 제90강. DPO — Preference를 직접 학습하기

### 1. 이번 강의에서 배울 것

제86~88강은 **Reward Model → PPO → KL**의 3단 파이프라인이었다. 강력하지만 무겁다. 모델이 여러 개 필요하고, 샘플링·가치 함수·하이퍼파라미터가 얽힌다.

**DPO(Direct Preference Optimization)** 는 질문 하나를 던진다.

> KL 제약이 있는 RL 목표의 **최적 정책**을 닫힌 형태로 풀면, 선호 데이터만으로 정책을 **직접** 학습할 수 있지 않은가?

이 강의를 마치면 다음을 말할 수 있어야 한다.

- Bradley–Terry 선호 모델과 RM의 관계
- KL-제약 보상 최대화의 최적 \(\pi^\star\)
- DPO 손실이 어떻게 RM을 **소거**하는지
- \(\beta\), chosen/rejected 로그확률의 역할
- DPO가 “RL이 아니다/맞다” 논쟁을 어떻게 이해하면 좋은지
- PPO 파이프라인 대비 장단점

유도는 생략 없이, 그러나 기호는 최소로 유지한다. 숫자 장난감으로 손실이 어느 방향을 미는지 확인한다.
