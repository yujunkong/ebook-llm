...
## 제98강. 4권 총정리 — Inference와 Serving으로

### 1. 이번 강의에서 배울 것

새 알고리즘을 거의 추가하지 않는다. 4권（79~97강）을 **한 장의 체크리스트와 배포 관점 지도**로 다시 접고, 5권의 입구인 **제99강. Training과 Inference의 차이**로 넘긴다.
...
챗봇 데모로 책을 끝내지 않는다. **학습으로 만든 정책 $\pi_\theta$를, 실제로 토큰을 스트리밍하는 시스템으로 넘기는 좌표**를 고정한다.

### 2. 왜 총정리가 Inference로 이어지는가
...
### 8. Training vs Inference — 미리 보는 경계（제99강 예고）
