...
## 제89강. KL Divergence의 역할

### 1. 이번 강의에서 배울 것

제88강까지 PPO로 정책을 갱신하는 뼈대를 잡았다. 그런데 PPO만 켜 두면 정책이 **보상만 쫓아 멀리 달아나** SFT로 쌓아 둔 언어·형식·안전 습관을 깨뜨리기 쉽다. 이번 강의는 그 브레이크가 되는 **KL Divergence(쿨백–라이블러 발산)** 를 다룬다.

이 강의를 마치면 다음을 말할 수 있어야 한다.

- \(\mathrm{KL}(\pi\Vert\pi_{\mathrm{ref}})\)가 무엇을 재는지, 왜 방향이 중요한지
- RLHF에서 \(\pi_{\mathrm{ref}}\)를 보통 **SFT 모델**로 두는 이유
...
#### 4.2 왜 \(\mathrm{KL}(\pi\Vert\pi_{\mathrm{ref}})\)인가
...
### 11. Forward KL과 Reverse KL — 한 단계 더
...
### 12. Trust Region 감각과 PPO clip의 역할 분담
