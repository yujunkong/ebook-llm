...
제87강에서 PPO clipped surrogate를 수식으로 이해했다. 이번 강의는 **돌아가는 최소 구현**을 만든다. 목표는 생산용 LLM 학습기가 아니라, 원리가 보이는 **토이 PPO 루프**다.
...
**사실:** 아래 코드는 교육용이며 특정 프레임워크의 성능 수치를 재현하지 않는다.  
**설명:** 단순화해도 clip의 핵심은 동일하다.
