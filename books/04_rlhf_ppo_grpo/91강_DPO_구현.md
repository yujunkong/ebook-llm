...
선호 마진을 $m$이라 하면 (교육용 요약):

$$
L_{\mathrm{DPO}} \approx -\log \sigma(\beta \, m)
$$

| $m$ | $\sigma(\beta m)$ ($\beta=0.1$ 감) | 손실 감 |
|---|---|---|
| 크게 + | 1에 가까움 | 작음 (이미 잘 구분) |
| 0 근처 | ~0.5 | 중간 |
| 음수 | 0에 가까움 | 큼 (선호를 반대로 믿음) |

실무 코드는 log-prob 합(응답 토큰)과 마스크를 정확히 맞춰야 합니다.
...
- **이전 강:** [90강. DPO — Preference를 직접 학습하기](90강_DPO_Preference를_직접_학습하기.md)
- **다음 강:** [92강. GRPO](92강_GRPO.md)
