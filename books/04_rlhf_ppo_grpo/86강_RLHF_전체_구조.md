...
제84강에서 Preference Dataset을, 제85강에서 Reward Model을 만들었다. 이번 강의는 부품을 **하나의 파이프라인**으로 조립한다. 이름하여 **RLHF(Reinforcement Learning from Human Feedback)**.

이 강의를 마치면 다음을 설명할 수 있어야 한다.
...
세부 수식(PPO clip)과 구현 루프는 제87·88강이다. 여기서는 **지도(map)**를 고정한다.
