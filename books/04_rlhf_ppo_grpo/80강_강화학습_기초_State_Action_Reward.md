...
제79강에서 Post-Training 지도를 펼쳤다.  
이번 강의부터는 그 지도 위의 RLHF·PPO·GRPO를 읽기 위한 **공통 언어**를 만든다.

강화학습（Reinforcement Learning, RL）전체 교과서를 쓰지 않는다.  
LLM 정렬에 필요한 최소 MDP（Markov Decision Process）조각만 고른다.
