...
제87~88강의 PPO는 **Actor + Critic(value)** 구조가 흔하다. 가치 함수는 분산을 낮추지만, LLM 스케일에서는 **메모리·구현 복잡도**가 커진다.

**GRPO(Group Relative Policy Optimization)** 계열 아이디어는 Roughly 이렇게 말한다.

> 같은 프롬프트에서 응답을 **그룹으로** 여러 개 샘플한 뒤, 그 안에서의 **상대적** 보상으로 advantage를 만들자. 그러면 별도 value 네트워크 없이도 정책 경사를 안정화할 수 있다.
...
중요: GRPO는 **빠르게 진화 중인 연구·실무 용어**다. 이 강의는 특정 버전을 영원한 표준으로 고정하지 않는다. DeepSeek 등에서 소개·사용된 형태를 **교육용 설명**으로 재구성한다. 하이퍼파라미터·클리핑·KL 배치·정규화는 원논문·코드가 우선이다.
