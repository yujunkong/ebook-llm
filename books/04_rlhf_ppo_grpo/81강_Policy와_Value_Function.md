...
제80강에서 $s_t,a_t,r_t$를 고정했다.  
이번 강의는 그 위에서 **두 종류의 함수**에 이름을 붙인다.

1. **Policy(정책)** — 상태에서 행동을 고르는 규칙（분포）
2. **Value Function(가치 함수)** — 상태가（또는 상태–행동 쌍이）얼마나 좋은지
...
### 9. PyTorch로 Policy · Value head 스케치
