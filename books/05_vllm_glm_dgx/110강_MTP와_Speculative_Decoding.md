...
표준 autoregressive 생성은 한 decode 스텝에 **토큰 하나**를 확정한다. GPU는 그 한 토큰을 위해 거대 가중치를 읽느라 **메모리 대역폭**에 묶이기 쉽다(제111강 예고). **Speculative Decoding**과 **MTP(Multi-Token Prediction)** 계열 아이디어는, 거칠게 말해 “한 번에 여러 토큰을 **제안**하고, 큰 모델로 **검증**해 여러 개를 한꺼번에 확정하자”는 쪽에 가깝다.
...
- 제107강 지표로 이득을 **정의·측정**하는 방법(숫자 날조 없이)

**사실:** speculative decoding·MTP·medusa류·draft model 등 명칭과 알고리즘 세부는 논문·엔진 플래그마다 다르다.  
**해석:** 이 강의는 **공통 골격**만 교육용으로 고정한다. 특정 모델이 “MTP를 쓴다”는 마케팅 문장을 그대로 속도 보증으로 읽지 말 것.
...
### 5. MTP — Multi-Token Prediction과의 관계
