...
## 제103강. Quantization — INT8, INT4, FP8

### 1. 이번 강의에서 배울 것

제102강까지는 **스케줄과 KV**로 GPU를 잘 쓰는 쪽을 봤다. 이번 강의는 메모·대역폭·（지원 시）연산 효율을 위해 숫자를 **낮은 비트로 표현**하는 **Quantization(양자화)** 를 다룬다. 초점은 서빙에서 자주 등장하는 **INT8, INT4, FP8**이다.
...
- 스케일·제로포인트 등 기본 수식
- 메모리 절감의 **이상적 비율**과 실제 이득이 어긋날 수 있는 이유（설명）
- 품질·하드웨어 지원·엔진 호환을 함께 보는 체크리스트
- 벤치마크 수치를 날조하지 않는 서술 습관

QLoRA（제74강）의 “4비트 가중치로 학습 가능” 직관과 연결하되, 오늘은 **추론 서빙** 관점이 중심이다.
...
#### 4.2 Mixed Precision Training과의 차이
...
#### 4.6 Weight-only vs Activation Quantization
...
#### 6.1 Uniform Affine Quantization
