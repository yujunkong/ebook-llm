...
q = \mathrm{clamp}\left(\mathrm{round}\big(\frac{x}{s}+z\big), q_{\min}, q_{\max}\right)
...
$$
q = \mathrm{clip}\left(\mathrm{round}\big(\frac{x}{s}\big)+z\right),\quad
\hat{x}=s(q-z)
$$
...
![그림 103-1](images/fig103-01.png)

파라미터 70B, FP16(2B/가중치) ≈ 140GB 감.  
INT4(0.5B)면 이론상 ≈ 35GB 급으로 줄어 **같은 GPU에 더 큰 모델**이 들어갑니다(실제는 오버헤드·스케일 파라미터 추가).
