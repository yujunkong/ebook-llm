...
제111~114강에서 GPU 병목, 엔진 선택, Tensor Parallel, NCCL·RoCE를 쌓았다. 이번 강의는 그 지식을 **책상 위 실습 환경**에 옮긴다. 대상은 **NVIDIA DGX Spark 2대**를 고속 링크로 묶은 소형 클러스터다.
...
목표가 아니다: 특정 시점의 비공개 벤치 숫자를 책에 고정하는 것.

목표가 맞다: **문서·플레이북·자체 측정**으로 환경을 재현 가능하게 구성하는 것.
