...
## 제107강. LLM Serving 성능 지표 — TTFT, TPOT, Throughput

### 1. 이번 강의에서 배울 것

제100~106강까지 Prefill/Decode, KV, Continuous Batching, vLLM 메모리·스케줄러를 쌓았다. 이제 “빠르다/느리다”를 **서로 다른 축의 숫자**로 분해한다.
...
**사실:** 용어 정의는 문헌·벤치 하네스마다 미세하게 다를 수 있다(예: TTFT에 큐잉 포함 여부).  
**해석:** 이 강의는 **교육용 공통 정의**를 고정한 뒤, 리포트에 “우리 정의는 X”를 명시하라고 요구한다. **벤치마크 수치를 이 책에 날조하지 않는다.** 아래 표의 값은 전부 `예시(미측정)` 자리표시다.
...
#### 4.2 TPOT — Time Per Output Token
...
### 13. Prefill tokens/s 와 Decode tokens/s 분리
