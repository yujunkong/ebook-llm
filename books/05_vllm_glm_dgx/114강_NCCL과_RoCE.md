...
제113강 Tensor Parallel은 “GPU들이 매 층마다 서로 이야기한다”로 끝났다. 그 이야기의 **통역·운송 계층**이 바로 **NCCL** 이고, 노드를 넘어갈 때 자주 등장하는 네트워크 기술이 **RoCE** 다.
...
- RoCE(RDMA over Converged Ethernet)의 위치를 **개요 수준**으로 설명한다.
- NCCL 디버깅에 쓰는 환경변수·체크 순서를 **처방전 남발 없이** 정리한다.
- 제115강 2× DGX Spark의 ConnectX·QSFP 링크를 이해할 준비를 마친다.

목표가 아니다: 네트워크 엔지니어 자격증 수준의 큐잉·PFC 튜닝 전과정.

목표가 맞다: TP hang / 느린 멀티노드 서빙을 볼 때 **어디를 먼저 의볼지** 아는 것.
...
#### 11.1 PyTorch가 NCCL을 쓰는지 확인 (스케치)
