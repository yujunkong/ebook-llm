...
제99~103강에서 서빙의 부품을 모았다.

```text
Training≠Inference → Prefill/Decode → KV Cache → Continuous Batching → Quantization
```

이번 강의는 그 부품들을 **하나의 Inference Engine**으로 묶는 대표 사례로 **vLLM**의 **공개 개념 수준 구조**를 본다.
...
**중요:** vLLM은 빠르게 변하는 오픈소스 프로젝트다. 클래스 이름·모듈 경로는 버전에 따라 달라질 수 있다. 이 강의는 **역할 지도**를 고정하고, 특정 커밋의 내부 구현을 **사실처럼 단정하지 않는다**.
...
### 10. LLM Serving 연결 — 부품이 엔진에서 만나는 곳
