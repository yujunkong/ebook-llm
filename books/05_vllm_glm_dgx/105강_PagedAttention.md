...
- Attention이 “논리적으로 연속인 KV”를 물리적으로 흩어진 블록에서 어떻게 읽는지
...
## Continuous Batching과 결합하는 메모리 시계열
한 스텝의 블록 회계(교육용):
...
블록 크기 $b=16$토큰일 때 길이 2000이면:

$$
n_{\mathrm{pages}}=\left\lceil 2000/16\right\rceil=125
$$

연속 큰 배열 대신 **페이지 테이블로 여기저기 있는 블록**을 가리키면, 할당·공유·프리엠션이 쉬워집니다(PagedAttention 직관).
