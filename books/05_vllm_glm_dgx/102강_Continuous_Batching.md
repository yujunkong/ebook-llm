...
제101강에서 KV Cache가 **요청마다 다른 속도·길이로 자란다**는 것을 봤다. 이번 강의는 그 위에서 GPU를 바쁘게 유지하는 스케줄 아이디어 — **Continuous Batching(연속 배치)** — 을 다룬다.
...
특정 엔진의 비공개 스케줄러 코드를 “사실”로 단정하지 않는다. **공개 개념·교육용 모델**로 설명한다.
