...
제100강에서 Prefill과 Decode를 나눴다. Decode가 싸지려면 **과거 토큰의 Key/Value를 다시 계산하지 않아야** 한다. 그 저장소가 **KV Cache(케이브이 캐시)** 다.
...
특정 GPU에서 “KV가 몇 GB”인지 **단정하지 않는다**. 식과 가정을 명시한다.
