...
## 제113강. Tensor Parallel

### 1. 이번 강의에서 배울 것

제111강에서 단일 GPU의 계산·대역폭 천장을, 제112강에서 엔진 선택을 보았다. 모델이 **한 디바이스 메모리에 들어가지 않거나**, 한 디바이스의 계산만으로는 SLA를 못 맞출 때 등장하는 기법이 **Tensor Parallel(TP, 텐서 병렬)** 이다.

이 강의를 마치면 다음을 할 수 있어야 한다.

- 가중치·활성화를 **여러 GPU에 샤딩**한다는 말이 층(layer) 안에서 무엇을 뜻하는지 설명한다.
- TP에 필요한 **집합 통신(collective)** 이 왜 생기는지 그림으로 말한다.
- TP가 도움이 되는 조건과, 오히려 느려질 수 있는 조건을 구분한다.
- Pipeline Parallel·Data Parallel과 **역할이 다름**을 표로 정리한다.
- 제114~115강의 NCCL·멀티노드 환경이 TP의 토대임을 연결한다.

목표가 아니다: 모든 병렬화 전략의 수식 전체를 유도하는 것.

목표가 맞다: 서빙 설정에서 `--tensor-parallel-size` 를 **의미 있게** 고르는 감각.
