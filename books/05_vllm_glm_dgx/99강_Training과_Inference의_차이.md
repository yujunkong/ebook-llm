...
## 수식 보강 — Train vs Infer 연산

학습: 보통 전체 시퀀스 teacher forcing + 역전파.

$$
L=-\sum_t\log p(x_t\mid x_{<t})
$$

추론: Prefill 후 토큰 단위 Decode. 동일 $p_\theta$지만 메모리 접근 패턴(KV cache)이 달라집니다.
...
| 목표 | Loss↓ | 토큰 생성 |
| 배치 | 큰 편 | 요청 단위·연속 배치 |
| 메모리 | 활성화+그라디언트+옵티마 | 가중치+KV |

같은 모델 파일이라도 **쓰는 방정식/자원**이 다릅니다.
