...
## 제120강. 5권·전권 총정리 — 밑바닥에서 Serving까지

### 1. 이번 강의에서 배울 것

새 기법을 거의 추가하지 않는다. 《밑바닥부터 LLM》 1~5권（1~119강）을 **한 줄 경로**로 다시 접고, 5권（99~119）을 Serving 관점의 체크리스트로 고정한다.
...
**챗봇 데모로 책을 끝내지 않는다.** 마지막 페이지의 산출물은 대화 스크린샷이 아니라, **밑바닥 개념이 Serving 결정으로 이어지는 지도**다.
...
#### 5.1 1권 — Python · Tensor · 수학 · PyTorch（1~26）
...
#### 5.2 2권 — Tokenizer와 Transformer（27~54）
...
#### 5.3 3권 — GPT Pretraining과 SFT（55~78）
...
#### 5.5 5권 — vLLM · GLM · DGX Spark（99~119）
...
- **이전 강:** [제119강. 실무 체크리스트와 장애 대응](119강_실무_체크리스트와_장애_대응.md)
- **다음 강:** 없음 (시리즈 끝)
