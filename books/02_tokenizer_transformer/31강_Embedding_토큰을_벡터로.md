...
<!-- visual-example-31 -->
## 숫자로 따라가기 — Embedding lookup

![그림 31-1](images/fig31-01.png)

어휘 크기 $|V|=4$, $d=3$인 장난감 표:
...
$$
\mathbf{e}_2 = E[2,:]=[0.1,\ -0.2,\ 0.5]
$$

| 기호 | 값 | 의미 |
|---|---|---|
| $\mathrm{id}$ | $2$ | 이산 토큰 번호 |
| $E$ | $4\times 3$ | Embedding 행렬 |
| $\mathbf{e}_{\mathrm{id}}$ | $[0.1,-0.2,0.5]$ | 연속 벡터 |
| one-hot $\mathbf{o}$ | $[0,0,1,0]$ | $\mathbf{e}=E^{\top}\mathbf{o}$와 동치 |

학습은 이 행을 움직입니다. 이후 위치 정보(PE/RoPE)가 더해져 Transformer 입력이 됩니다.
