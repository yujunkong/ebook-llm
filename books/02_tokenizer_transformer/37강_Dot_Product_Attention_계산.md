...
36강에서 Query, Key, Value를 만들었다.  
이번 강의는 Attention의 점수판을 숫자로 채운다.

이 강의를 마치면 다음을 말할 수 있어야 한다.

- **Dot-Product Attention**에서 점수가 \(\mathbf{q}\cdot\mathbf{k}\)인 이유
- 행렬 형태 \(S = QK^\top\)의 Shape와 한 칸의 의미
- **Scaled Dot-Product**: \(S = QK^\top / \sqrt{d_k}\)와 스케일이 필요한 이유
- 토큰 2~3개, 차원 2짜리로 **손계산**하기
- Softmax 전 단계까지를 코드로 재현하기

38강에서 Softmax와 Value 가중합을 붙이면, 한 헤드 Attention의 순전파가 완성된다.
