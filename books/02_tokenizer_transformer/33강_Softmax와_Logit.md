...
32강에서 Language Model은 \(P(x_{t+1}\mid x_{\le t})\)를 낸다고 했다. 그런데 신경망의 마지막 Linear 층이 직접 확률을 출력하는 경우는 드물다. 보통은 **점수(logit)**를 먼저 만들고, **Softmax**로 확률로 바꾼다.

이 강의를 마치면 다음을 말할 수 있어야 한다.

- **Logit**과 **Softmax**의 정의·동기
- Softmax가 만족하는 확률 성질 (\(>0\), 합=1)
- 벡터 `[2, 1, 0]`으로 Softmax를 **손계산**하기
- **Temperature**가 분포의 뾰족함을 어떻게 바꾸는지 (미리보기)
- 큰 숫자에서 생기는 수치 불안정과 안전한 계산법
- LLM 생성·학습에서 Softmax가 어디에 붙는지

다음 34강 Cross Entropy는 Softmax 확률과 정답 토큰을 잇는 Loss이다.
...
### 12. Softmax vs Sigmoid vs Hardmax
...
- **이전 강:** [제32강. Language Model과 Next Token Prediction](32강_Language_Model과_Next_Token_Prediction.md)
- **다음 강:** [제34강. Cross Entropy Loss](34강_Cross_Entropy_Loss.md)
