...
맵 $A\in\mathbb{R}^{T\times T}$에서 $A_{ij}=\alpha_{ij}$는 “쿼리 $i$가 키 $j$에 둔 무게”입니다.

$$
\sum_j A_{ij}=1,\qquad A_{ij}\ge 0
$$

Causal이면 $j>i$인 칸은 0에 가깝습니다(마스킹).

$$
A_{ij}=0\quad(j>i)
$$

헤드가 여러 개면 헤드마다 다른 $A^{(h)}$를 갖습니다. 시각화는 **한 헤드·한 샘플**부터 보는 편이 안전합니다.


<!-- visual-example-51 -->
## 숫자로 따라가기 — Attention 히트맵

![그림 51-1](images/fig51-01.png)

토큰 A B C, Causal mask 후 Softmax 가중치(행=Query):
...
| 행 A | 미래 차단 → 자기만 |
| 행 B | 과거·자기를 반반 |
| 열 C (위쪽) | A·B는 C를 못 봄 (causal) |

히트맵은 $\alpha_{ij}$를 색으로 본 것입니다. “어디에 주목했는지” 디버깅·해석의 기본 도구입니다.
