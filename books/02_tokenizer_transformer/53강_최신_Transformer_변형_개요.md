...
## 워크드 예제 — KV 절감·MoE 게이트·창 주의

### KV 숫자

$N=32,H_q=32,d=128,T=4096$, FP16($b=2$).

MHA ($H_{\mathrm{kv}}=32$):

$$
\mathrm{Mem}=2\cdot32\cdot32\cdot4096\cdot128\cdot2
\approx 2.15\times10^9\ \mathrm{B}
\approx 2.0\,\mathrm{GB}
$$

(설명용; 페이지·정렬 오버헤드 제외.)

GQA $H_{\mathrm{kv}}=8$: 같은 식에 $8$ → 약 $0.5\,\mathrm{GB}$ (비율 $1/4$).

### MoE 게이트 손계산

$E=4$, $g=(1.0, 3.0, 0.0, 2.0)$, $k=2$.  
top2 = 전문가 2와 4 ($g=3,2$).

$$
\pi_2=\frac{e^{3}}{e^{3}+e^{2}}\approx0.731,\quad
\pi_4\approx0.269
$$

$$
y\approx0.731\,\mathrm{FFN}_2(x)+0.269\,\mathrm{FFN}_4(x)
$$

### Sliding window $w=4$, $T=10$

위치 7이 보는 키: $4,5,6,7$ (창) — 연결 4개.  
전체 causal이면 8개($0..7$). $T$가 커질수록 $w$ 고정 창의 상대 이득이 커진다.
