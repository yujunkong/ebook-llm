...
### 3.2 Negative Log Likelihood와의 관계
...
### 5.2 왜 Softmax와 잘 맞는가 (Gradient 스케치)
...
<!-- visual-example-34 -->
## 숫자로 따라가기 — Cross-Entropy

![그림 34-1](images/fig34-01.png)

위에서 만든 $p\approx[0.66,0.24,0.10]$이고 정답이 첫 토큰(index 0)이면:

$$
L=-\log 0.66 \approx 0.42
$$

같은 Softmax인데 정답 확률이 $0.05$로 망가지면:

$$
L=-\log 0.05 \approx 3.00
$$

한 줄 요약: **정답을 낮게 믿을수록 Loss가 커지고**, 학습은 그 확률을 올리는 쪽으로 $\theta$를 움직입니다.
...
### 9.4 Softmax를 학습 루프에서 따로 쓰지 않는 이유
