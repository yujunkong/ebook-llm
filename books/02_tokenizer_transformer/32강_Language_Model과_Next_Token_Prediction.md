...
Tokenizer가 id를 만들고 Embedding이 벡터를 만든다. 그런데 LLM은 이 벡터로 **무엇을 최적화**하는가?
...
- **Teacher Forcing**의 개념(상세 Loss는 34강)
- GPT 파이프라인에서 “이해/생성”이 같은 목표에서 나오는 이유

33강 Softmax, 34강 Cross Entropy가 바로 이 목표의 수치화이다.
...
#### 4.3 Next Token Prediction (다음 토큰 예측)
...
#### 4.4 Autoregressive Language Model
...
#### 4.5 Teacher Forcing (티처 포싱) 미리보기
...
### 9. 코드로 구현하기 — 초소형 Autoregressive 루프
