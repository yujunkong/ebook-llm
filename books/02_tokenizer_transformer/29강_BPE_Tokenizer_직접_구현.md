...
초기 어휘=문자. 매 스텝 가장 빈도 쌍 $(a,b)$를 병합합니다.

$$
(a,b)\mapsto ab
$$

$M$번 병합하면 어휘 크기는 대략 $|\Sigma|+M$입니다. 학습은 코퍼스 통계로 병합 순서를 정하는 일입니다.


## 수식 보강 — Merge 횟수와 Vocab 크기

초기 기호 집합 크기 $|V_0|$（문자 또는 바이트）에 merge를 $N$번 적용하면, 새 기호가 하나씩 늘어

$$

|V| \approx |V_0| + N_{\mathrm{merge}}
$$

입니다（special token·UNK·중복 병합 실패는 별도）.  
예: $|V_0|=27$（a–z + `</w>`）, $N=100$ → $|V|\approx 127$.
...
```text
low</w> low</w> lowest</w> newer</w> wider</w>
```

초기 분할 후 인접 쌍 빈도를 세면（설명용 가정）`e r`, `l o` 등이 높게 나옵니다.  
1회 merge로 `e r → er`이면 서열이 짧아지고, 2·3회에서 `lo`, `low`가 생길 수 있습니다.  
**학습은 전역 빈도 순서**, 인코딩은 그 표의 고정 순서를 따릅니다. 인코딩 중에 빈도를 다시 고르지 않습니다.

### 효율 감각

같은 문자열의 토큰 수 $T(N)$은 보통 $N$이 커질수록 감소하다가 포화합니다.  
미니 실험에서는 $N\in\{5,10,20\}$만 바꿔 $T$와 $|V|$를 표로 남기면 충분합니다. 공개 리더보드 숫자를 만들지 마세요.
...
- **이전 강:** [28강. Character / Word / Subword Tokenization](28강_Character_Word_Subword_Tokenization.md)
- **다음 강:** [30강. Vocabulary와 Special Tokens](30강_Vocabulary와_Special_Tokens.md)
