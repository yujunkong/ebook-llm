...
## 제36강. Query, Key, Value

### 1. 이번 강의에서 배울 것

35강에서 Attention이 **가중 평균으로 문맥을 섞는다**는 그림을 그렸다.  
이번 강의는 그 가중치와 가중합에 이름을 붙인다.

이 강의를 마치면 다음을 말할 수 있어야 한다.

- **Query(질의), Key(키), Value(값)** 각각의 역할
- 도서관·검색 비유로 Q/K/V를 설명하기
- Self-Attention에서 Q, K, V가 **같은 입력의 선형 변환**으로 나온다는 것
- 기호 \(Q=XW_Q,\ K=XW_K,\ V=XW_V\)를 Shape와 함께 쓰기
- 37강의 \(QK^\top\)로 넘어갈 준비 마치기

Attention 공식의 “부품 목록”이 오늘이다. 점수 계산·Softmax·마스크는 이어지는 강의에서 하나씩 붙인다.
