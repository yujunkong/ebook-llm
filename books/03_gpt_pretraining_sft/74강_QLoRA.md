...
제73강 LoRA는 **학습 파라미터**를 줄였다. 그러나 순전파에 필요한 **베이스 가중치 $W_0$ 자체**는 여전히 GPU 메모리에 올라와야 한다. QLoRA(Quantized LoRA)는 그 베이스를 **저비트로 올려 두고**, 그 위에서 LoRA 어댑터만 학습하는 조합이다.

이번 강의의 목표는 “라이브러리 한 줄 마법”이 아니라 다음을 구분하는 것이다.
...
- 가짜 VRAM 숫자를 단정하지 않고, **예시 스케일**로만 감각을 잡는다.
...
- **이전 강:** [제73강. LoRA](73강_LoRA.md)
- **다음 강:** [제75강. SFT 평가와 실패 사례](75강_SFT_평가와_실패_사례.md)
