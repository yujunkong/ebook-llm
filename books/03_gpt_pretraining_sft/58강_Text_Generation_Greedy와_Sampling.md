...
### 워크드 예 — 같은 logit, 다른 정책

$\mathbf{z}=(3.0,\ 2.0,\ 0.5,\ -1.0)$. Softmax($\tau=1$):

$$

\mathbf{p}\approx (0.644,\ 0.237,\ 0.053,\ 0.012)
$$

| 정책 | 선택 |
|---|---|
| Greedy | 인덱스 0 (확정) |
| Sample $\tau=1$ | 0이 자주, 1이 가끔 |
| $\tau=0.5$ (미리보기) | $z/\tau=(6,4,1,-2)$ → 0에 더 몰림 |
| $\tau=2$ (미리보기) | 분포가 평평해져 2·3도 더 자주 |

Top-$k=2$: 남기는 집합 $\{0,1\}$, 재정규화 후

$$

p'_0=\frac{0.644}{0.644+0.237}\approx 0.731,\quad
p'_1\approx 0.269
$$

인덱스 2·3은 확률 0. **오늘의 본문은 $k=\infty$(절단 없음) sampling과 greedy**다. $\tau$·top-k/p는 제59강.

### 경로 확률 — greedy가 전역 최적이 아닌 장난감

어휘 $\{A,B,C\}$, 두 스텝만 생성. 조건부 확률(설명용):

```text
P(A|prompt)=0.4,  P(B|prompt)=0.35,  P(C|prompt)=0.25
P(A|A)=0.1,       P(B|A)=0.1,        P(C|A)=0.8
P(A|B)=0.6,       P(B|B)=0.3,        P(C|B)=0.1
```

Greedy 경로: $A$ 다음 $C$ → 경로 확률 $0.4\times 0.8=0.32$.  
대안 $B$ 다음 $A$: $0.35\times 0.6=0.21$ (더 작음).  
이 숫자에서는 greedy가 이겼지만, $P(A|\mathrm{prompt})$만 약간 바꾸고 $P(*|A)$를 평평하게 만들면 **첫 스텝에서 2등이 전체 곱을 이길** 수 있다. 그래서 빔 서치 동기가 생긴다.

### 엔트로피와 “얼마나 고르지 않은가”

$$

H(\mathbf{p}) = -\sum_v p_v\log p_v
$$

Greedy는 $H$를 쓰지 않고 mode만 본다. Sampling은 $H$가 큰 위치에서 더 다양한 토큰이 나온다.  
학습이 덜 된 모델은 $H$가 전반적으로 커서 sampling이 붕괴하기 쉽다.
