...
제56강에서 GPT가 `logits [B, T, V]`를 내게 했다. 이번 강의는 그 logits 위에서 **무엇을 최적화하는지**를 고정한다.

이 강의를 마치면 다음을 할 수 있어야 한다.

- Causal LM 목표 $\max \sum_t \log p(x_t\mid x_{<t})$를 문장·수식으로 쓴다
- 입력 $x$와 라벨 $y$를 **한 칸 시프트**로 맞춘다
...
- Pretraining 루프의 “한 스텝”을 코드로 그린다
...
$$
L(\theta)=\mathbb{E}_{x\sim\mathcal{D}}\Big[-\sum_{t}\log P_\theta(x_t\mid x_{<t})\Big]
$$
