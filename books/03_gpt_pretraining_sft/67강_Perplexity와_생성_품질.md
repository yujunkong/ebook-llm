...
제66강에서 Validation loss와 Evaluation의 자리(언제 재고, 무엇을 고정하고, 무엇을 비교하는지)를 정리했다. 이번 강의는 그 숫자 중 하나인 **Perplexity(PPL, 퍼플렉시티)**를 끝까지 파고, **생성 품질**과 어떻게 다른지 구분한다.

이 강의를 마치면 다음을 말할 수 있어야 한다.

- PPL이 **평균 NLL(Negative Log Likelihood)의 지수**임을 식으로 쓰기
...
다음 **제68강. 프로젝트 — Mini GPT Pretraining**에서 이 지표를 실제로 학습 로그에 붙인다.
...
#### 6.2 Bits-per-character / Bits-per-byte와의 관계
