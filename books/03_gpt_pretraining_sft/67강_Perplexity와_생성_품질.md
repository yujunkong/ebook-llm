...
입니다（로그 밑이 $e$일 때）. $\log_2$로 NLL을 정의하면 $2^{L}$이 됩니다. **밑을 섞지 마세요.**

### 해석 스케치

균등 모델 $p=1/V$이면 $L=\log V$, $\mathrm{PPL}=V$입니다.  
$V=1000$이면 PPL 1000은 “아직 거의 무작위” 감각입니다. 데이터·토크나이저에 따라 체감이 달라 **절대 컷오프를 업계 표준처럼 단정하지 않습니다.**

## 부록. 생성 품질과 PPL의 분리

| 낮은 PPL | 높은 생성 품질 |
|---|---|
| 다음 토큰 예측 좋음 | 독자·과제 기준의 유용성 |
| 자동·저렴 | 주관·비쌀 수 있음 |
| 지시 준수 보장 없음 | harness/인간이 필요 |

SFT 단계로 가면 PPL만으로 성공을 선언하지 않습니다（제75강）.
...
$$
\mathrm{PPL}=e^{2.3}\approx 10
$$

“매 토큰 평균 10지선다처럼 헷갈린다”는 거친 해석입니다. 낮을수록 예측이 뾰족.
...
- **이전 강:** [66강. Validation과 Evaluation](66강_Validation과_Evaluation.md)
- **다음 강:** [68강. 프로젝트 — Mini GPT Pretraining](68강_프로젝트_Mini_GPT_Pretraining.md)
