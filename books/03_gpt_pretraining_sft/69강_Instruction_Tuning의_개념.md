...
프롬프트 길이 $L_p$, 응답 길이 $L_r$, 전체 $T=L_p+L_r$이라 하자.

- Pretraining식(전부 합산): 항이 $T$개
- SFT(응답만): 항이 $L_r$개

평균을 $L_r$로 나누면, 긴 지시에 짧은 답만 있는 샘플이 **응답 품질에 더 민감**해진다.  
반대로 프롬프트 토큰까지 CE를 주면, 모델이 “사용자 문장 복사”에 용량을 쓸 수 있다.
...
같은 배치라도 마스크에 따라 그라디언트 스케일이 달라지므로, 구현에서 `sum`/`mean`과 ignore_index를 명확히 한다(제71강).
