...
제71~72강에서 SFT(Supervised Fine-Tuning)의 목표와 chat template을 다뤘다. 문제는 단순하다. **사전학습된 거대 가중치 전체를 매번 다시 학습하면** 메모리·저장·실험 비용이 너무 크다.

LoRA(Low-Rank Adaptation)는 그 비용을 줄이는 **파라미터 효율적 미세조정(PEFT)** 기법이다. 이번 강의는 라이브러리 사용법보다 **행렬이 왜 작아도 되는지**를 숫자로 잡는다.
